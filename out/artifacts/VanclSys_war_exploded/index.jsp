...
<head>
    ...
    <%
        ...
        int sum=gd.findCartNum();
        request.setAttribute("sum",sum);
        List<CarGoods> list=gd.getShoppingCart();
        request.setAttribute("shoppingCar",list);
        ...
        <div class="search-bar">
            <div class="search-tab">
                ...
                <div class="active" id="shoppingCar">
                    <p><a id="shoppingCar-link" class="car-tab" href="pages/shoppingCart.jsp">购物车(<span class="car-number">${sum}</span>)</a>
                    </p>
                    <div class="bottomline"></div>
                    <c:if test="${shoppingCar.size()==0}">
                        <div class="car-list">
                            <p>您的购物车中没有任何商品</p>
                        </div>
                    </c:if>
                    <c:if test="${shoppingCar.size()!=0}">
                        <div class="car-list" >
                            ...
                            <c:forEach items="${shoppingCar}" var="item">
                                <div>
                                    <div style="width: 220px;padding: 5px;overflow: hidden;float: left">
                                        <img src="images/12.jpg" width="36" height="36" >
                                        <p style="float: left;margin-bottom: 0px;">${item.gname}<br>
                                            <span style="color: #a10000">￥${item.gprice}×${item.gcount}</span>
                                        </p>
                                    </div>
                                    <a class="del" style="color:#a10000;float:right;margin-top: 15px">删除</a>
                                    <input type="hidden" value="${item.gid}" name="GoodsId" id="Gid">
                                    <hr style="clear: both;margin: 0 0">
                                </div>
                            </c:forEach>
                        <a class="btn btn-info" style="margin-top: 20px;float: right;border: none;border-radius: 0px;background-color: #B81C22" href="pages/shoppingCart.jsp">查看购物车</a>
...
// 鼠标移上实现子菜单的显现隐藏
        ...
        $(".del").click(function () {
            ...
            $(this).parent().remove();
            $(".car-number").text($(".car-number").text()-1);
            if( $(".car-number").text() == 0){
                $(".car-list").empty();
                $(".car-list").append("<p>您的购物车没有任何商品！</p>");
            }
            $.ajax({
                url:"/delShoppingCart",
                type:"post",
                data:{"GoodsId":$(this).next().val()},
